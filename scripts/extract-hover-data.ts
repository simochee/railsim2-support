 ...
 *
 * Parses RailSim II help HTML files and extracts structured documentation
 * for use in the LSP hover provider.
 *
 * Output: src/server/hoverData.generated.ts
 *
...
/** Extract all argname→argexp pairs from a page */
function extractPropertyDocs(html: string): Map<string, string> {
  const props = new Map<string, string>();
  // Match pairs: <p class="argname">NAME</p> ... <p class="argexp">DESC</p>
  const regex =
    /<p\s+class="argname">\s*([^<]+?)\s*<\/p>\s*<p\s+class="argexp[^"]*">([\s\S]*?)<\/p>/gi;
...
function main() {
  ...
  const symbolDocs: Record<string, SymbolDoc> = {};
  ...
  for (const [filename, objectNames] of Object.entries(FILE_TO_OBJECTS)) {
    const filePath = resolve(HELP_SRC, filename);
