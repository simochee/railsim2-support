...
export class SymbolTable {
	...
	static fromHelpDir(helpDir: string): SymbolTable {
		...
		const files = fs
			.readdirSync(helpDir)
			.filter((f) => f.startsWith("pi_") && f.endsWith(".html"));
	 ...
	 */
	private registerTopLevelObjects(
		rules: GrammarRule[],
	...
	private expandUnionBlock(
		...
		for (const objName of rule.objectNames) {
			const effectiveParent =
				parentObjectName &&
				parentObjectName !== "__ref__" &&
				parentObjectName !== "__root__"
					? parentObjectName
					: undefined;
			const schemaKey = effectiveParent
				? `${objName}:${effectiveParent}`
				: undefined;
