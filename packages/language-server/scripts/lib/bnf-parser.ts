...
class Parser {
	 ...
	 */
	private parsePropertyAfterEquals(
		name: string,
		optional: boolean,
	): GrammarRule {
		const valueTokens: Token[] = [];
	...
	private buildProperty(
		...
		if (hasCtrlGroup && types.length > 0) {
			...
			const type =
				uniqueTypes.length === 1 ? uniqueTypes[0] : "expression";
			return {
				kind: "property",
				name,
				type,
	...
	private tryExtractEnum(tokens: Token[]): string[] | null {
		...
		if (!inner.some((t) => t.kind === "ctrl" && t.value === "|"))
			return null;
	...
	private parseGroupedWithQuantifier(
		...
		const hasPipe = inner.some(
			(t) => t.kind === "ctrl" && t.value === "|",
		);
