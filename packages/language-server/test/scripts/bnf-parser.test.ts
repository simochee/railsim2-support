...
describe("parseBnfBody", () => {
  ...
  it("parses a symbol reference with * quantifier", () => {
    const html = '<a class="nonterm">platform</a><span class="sup">*</span>';
    const result = parseBnfBody(html);
    expect(result).toEqual([{ kind: "ref", symbol: "platform", min: 0, max: Infinity }]);
  });

  it("parses a symbol reference with 1+ quantifier", () => {
    const html = '<a class="nonterm">axle-object</a><span class="sup">1+</span>';
    const result = parseBnfBody(html);
    expect(result).toEqual([{ kind: "ref", symbol: "axle-object", min: 1, max: Infinity }]);
  });

  it("parses a bare symbol reference (no quantifier)", () => {
    const html = '<a class="nonterm">plugin-header</a>';
    const result = parseBnfBody(html);
    expect(result).toEqual([{ kind: "ref", symbol: "plugin-header", min: 1, max: 1 }]);
  });
...
<span class="ctrl">|</span> <a class="nonterm">windmill</a>`;
  ...
  it("parses a union-property ((NoCastShadow|...) = integer (,integer)*;)", () => {
    ...
    expect(result).toEqual([
      {
        kind: "union-property",
        names: ["NoCastShadow", "NoReceiveShadow", "NoShadow", "Transparent"],
        type: "integer",
        optional: false,
        multiple: true,
        arity: 1,
  ...
  it("parses an arity-2 property (Lifetime = float, float;)", () => {
    const html = 'Lifetime = <a class="nonterm">float</a>, <a class="nonterm">float</a>;';
    const result = parseBnfBody(html);
    expect(result).toEqual([
      {
        kind: "property",
