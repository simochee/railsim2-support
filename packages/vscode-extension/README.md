...
RailSim2 のプラグイン定義ファイルは Shift_JIS（SJIS）で保存する必要があります。VS Code は初期設定では UTF-8 でファイルを開くため、日本語が文字化けすることがあります。以下の設定で Shift_JIS を扱えるようにしましょう。
...
毎回文字コードを指定するのが手間な場合は、設定でデフォルトの文字コードを変更できます。

1. `Ctrl+,`（Mac: `Cmd+,`）で設定を開く
2. 検索欄に `Encoding` と入力する
3. **Files: Encoding** を `shiftjis` に変更する

> **注意:** デフォルトの文字コードを変更すると、他のプロジェクトのファイルにも影響します。RailSim2 のプラグイン開発専用のワークスペースを作り、ワークスペース設定で変更することをおすすめします。

## 対応バージョン

RailSim 2.14 のプラグイン定義ファイルを対象にしています。
RailSim2 -k-build の独自記法には対応していません。

## 不具合・要望

不具合や機能追加の要望は [GitHub Issues](https://github.com/simochee/railsim2-support/issues) へお願いします。
